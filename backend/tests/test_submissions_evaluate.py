...
def evaluable_submission(db_session, make_submission):
    """A draft submission with one screenshot artifact attached."""
...
def test_admin_evaluate_flips_to_processing_and_sends_full_payload(
    ...
    assert payload["rubric_unique_name"] == rub.unique_name
    ...
    assert payload["callback_url"].endswith("/webhooks/n8n-callback")
    decoded = decode_callback_token(payload["callback_token"])
    assert decoded == sub.id

    types = {a["type"] for a in payload["artifacts"]}
    assert types == {"github_link", "screenshot"}
    screenshot = next(a for a in payload["artifacts"] if a["type"] == "screenshot")
    assert screenshot["filename"] == "shot.png"
    assert "/artifacts/" in screenshot["url"]
    github = next(a for a in payload["artifacts"] if a["type"] == "github_link")
...
def test_evaluate_non_draft_returns_409(
...
def test_n8n_failure_returns_502_and_keeps_draft(
...
def test_unauthenticated_cannot_evaluate(
    client, evaluable_submission, make_user
):
    other_admin, _ = make_user(role="admin")
    _, _, sub = evaluable_submission(created_by=other_admin.id)
